...
let points = 0;
let flag = true;
const imgs_Instructions = ["דפי הוראות/2.jpg", "דפי הוראות/3.jpg", "דפי הוראות/4.jpg", "דפי הוראות/5.jpg", "דפי הוראות/6.jpg", "דפי הוראות/7.jpg", "דפי הוראות/8.jpg", "דפי הוראות/9.jpg", "דפי הוראות/10.jpg", "דפי הוראות/11.jpg", "דפי הוראות/12.jpg", "דפי הוראות/13.jpg", "דפי הוראות/14.jpg", "דפי הוראות/15.jpg", "דפי הוראות/16.jpg"];
const card = ["כרטיסים/1.jpg", "כרטיסים/2.jpg", "כרטיסים/3.jpg", "כרטיסים/4.jpg", "כרטיסים/5.jpg", "כרטיסים/6.jpg", "כרטיסים/7.jpg", "כרטיסים/8.jpg", "כרטיסים/9.jpg", "כרטיסים/10.jpg", "כרטיסים/11.jpg", "כרטיסים/12.jpg", "כרטיסים/13.jpg", "כרטיסים/14.jpg", "כרטיסים/15.jpg"];
const cardAnswer = ["פתרונות/a1.jpg", "פתרונות/a2.jpg", "פתרונות/a3.jpg", "פתרונות/a4.jpg", "פתרונות/a5.jpg", "פתרונות/a6.jpg", "פתרונות/a7.jpg", "פתרונות/a8.jpg", "פתרונות/a9.jpg", "פתרונות/a10.jpg", "פתרונות/a11.jpg", "פתרונות/a12.jpg", "פתרונות/a13.jpg", "פתרונות/a14.jpg", "פתרונות/a15.jpg"];
const answer = ["null", "null", "null", "null", "null", "null"];
const all_answers = [
...
if (localStorage.getItem('level') == undefined)//אם לא נשמר והלוקאלסטורג ריק אז תפתח ותקרא לו לבל1
    localStorage.setItem('level', "level1");
if (localStorage.getItem('point') == undefined)
    localStorage.setItem('point', 0);//מעדכן את הנקודות ל0
if (localStorage.getItem('indexStep') == undefined)
    localStorage.setItem('indexStep', 0);//מעדכןמספר צעדים ל0
function StartLevels() {
    document.getElementById("imgAudio").addEventListener("click", Audio);//יקרא  לאודיו המוזיקה כאשר יקליק עליו
    document.getElementById("level1").addEventListener("click", EnterLevel);//יכנס לפונקציה לדף הרמה שהקליקו עליו
...
function EnterLevel() {
    window.location.href = "game.html";//נכנס לעמוד המשחק
    localStorage.setItem('level', event.target.id);//שומר את הרמה שנבחרה במשחק
}
function closeGame() {//סגירת החלון הפועל
    window.close();
}
function start() {
    document.getElementById("imgAudio").addEventListener("click", Audio);//יקרא לפונקציה אודיו מוזיקה כאשר יקליק
    let checkLevel = localStorage.getItem('level');//שומרת את הרמה במשתנה צק לבל
    let changeLevel = document.getElementById("level");//שומרת את אלמנט הלבל במשתנה
    if (localStorage.getItem('game') == "old") {//בודק אם יש משחק שמור
        index_Step = localStorage.getItem('indexStep');//קוראת את הערך השמור ומכניסה לאינדקס סטפ
        points = localStorage.getItem('point');//קוראת את הערך השמור ומכניסה לנקודות
        changeLevel.innerText = localStorage.getItem('level');//משנה את הטקס עם הערך של הרמה שנבחר
        document.getElementById("points").innerText = points;//משנה את הטקס עם הערך של מספר הנקודות
        document.getElementById("card_img").setAttribute("src", card[index_Step]);// מעדכנת את הכרטיס לכרטיס הבא במערך כרטיסים
    }
    else {
        localStorage.setItem('point', 0);//אם לא קיים משחק שמור מעדכנת התחלה
        localStorage.setItem('indexStep', 0);
        points = 0;
        if (checkLevel == "level1") {
            index_Step = 0;
            changeLevel.innerText = "level1";//מעדכנת את הטקסט לערך הרמה
        }
